...
function FactsScreen() {
  ...
  function getDrugInfo(drugName) {
    ...
    if (!!drugData["aliases"]) {
      ...
      var aliasString = ""
      for (var i = 0; i < drugData["aliases"].length; i++) {
    ...
    if (!!drugData["tolerance"]) {
      dosage += "\nTolerance" + "\n"
      if (!!drugData["tolerance"].full){dosage += "Full: " + drugData["tolerance"].full + "\n"}
      if (!!drugData["tolerance"].half){dosage += "Half: " + drugData["tolerance"].half + "\n"}
      if (!!drugData["tolerance"].zero){dosage += "Zero: " + drugData["tolerance"].zero + "\n"}
      if (!!drugData["crossTolerances"]){dosage += "Cross Tolerances:\n" + drugData["crossTolerances"] + "\n"}
    }
    ...
    var content = []
    if (summary != "") {content.push({ title: 'Summary', content: summary })}
    if (dosage != "") {content.push({ title: 'Dosage', content: dosage })}
    if (combos != "") {content.push({ title: 'Interactions', content: combos })} 
    setCONTENT(content);
